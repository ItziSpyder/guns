...
public class AssaultScope extends Scope {
    ...
    public void spawn() {
        ...
        v1 = rotVec(center.clone().add(radius, -radius, 0).toVector(), pitch, -yaw, center.toVector());
        v2 = rotVec(center.clone().add(-radius, -radius, 0).toVector(), pitch, -yaw, center.toVector());
        base = newPartBase(toLoc(v1, w));
        part = addPart(BlockDisplayRaytracer.trace(Material.GRAY_CONCRETE, toLoc(v1, w), toLoc(v2, w), 0.006, stay));
        base.addPassenger(part);

        v1 = rotVec(center.clone().add(radius, -radius, 0).toVector(), pitch, -yaw, center.toVector());
        v2 = rotVec(center.clone().add(radius, radius / 2, 0).toVector(), pitch, -yaw, center.toVector());
        v3 = rotVec(center.clone().add(-radius, -radius, 0).toVector(), pitch, -yaw, center.toVector());
        v4 = rotVec(center.clone().add(-radius, radius / 2, 0).toVector(), pitch, -yaw, center.toVector());
        base = newPartBase(toLoc(v1, w));
        part = addPart(BlockDisplayRaytracer.trace(Material.GRAY_CONCRETE, toLoc(v1, w), toLoc(v2, w), 0.006, stay));
        base.addPassenger(part);
        base = newPartBase(toLoc(v3, w));
        ...
        v1 = rotVec(center.clone().add(radius, radius / 2, 0).toVector(), pitch, -yaw, center.toVector());
        v2 = rotVec(center.clone().add(radius * 0.8, radius, 0).toVector(), pitch, -yaw, center.toVector());
        v3 = rotVec(center.clone().add(-radius, radius / 2, 0).toVector(), pitch, -yaw, center.toVector());
        v4 = rotVec(center.clone().add(-radius * 0.8, radius, 0).toVector(), pitch, -yaw, center.toVector());
        base = newPartBase(toLoc(v1, w));
        part = addPart(BlockDisplayRaytracer.trace(Material.GRAY_CONCRETE, toLoc(v1, w), toLoc(v2, w), 0.006, stay));
        base.addPassenger(part);
        base = newPartBase(toLoc(v3, w));
        ...
        v1 = rotVec(center.clone().add(radius * 0.8, radius, 0).toVector(), pitch, -yaw, center.toVector());
        v2 = rotVec(center.clone().add(-radius * 0.9, radius, 0).toVector(), pitch, -yaw, center.toVector());
        base = newPartBase(toLoc(v1, w));
        part = addPart(BlockDisplayRaytracer.trace(Material.GRAY_CONCRETE, toLoc(v1, w), toLoc(v2, w), 0.006, stay));
        base.addPassenger(part);
